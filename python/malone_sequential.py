...
class malone_streaming:
    ...
    def buildOriginalExecLine(self, nIndex):
        ...
        if line:
            ...
            if nFactorTime == 0:
                nFactorTime = 10
            if self.environment.MarkerToken != "":
                line = "timeout "+str(float(nFactorTime))+" "+linePattern + line+ " | grep '"+self.environment.MarkerToken+"'";
            else:
        ...
        while nIndexMutant < nEndMutant:
            ...
            while ((nIndexTc < nTestEnd) and bContinue):    
                ...
                if output and output == self.originalResults[nIndexTc]:
                    ...
                    self.nMutantsKilled +=1
                    print("Mutant killed: ", nIndexMutant, " | out: ", output, " vs exp: ", self.originalResults[nIndexTc])
